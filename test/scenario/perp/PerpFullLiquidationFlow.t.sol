...
contract ScenarioERC20Decimals is ERC20 {
...
contract ScenarioOracle is IOracle {
    struct PriceData {
        uint256 price;
        uint256 updatedAt;
        bool ok;
...
contract ScenarioPerpRiskModule is IPerpRiskModule {
...
contract ScenarioCollateralSeizer is ICollateralSeizer {
    struct Preview {
...
contract ScenarioInsuranceFund {
...
contract PerpFullLiquidationFlowTest is Test {
    ...
    ScenarioInsuranceFund internal insuranceFund;
    ScenarioERC20Decimals internal usdc;
    ScenarioERC20Decimals internal weth;
    ...
    function setUp() external {
        ...
        oracle = new ScenarioOracle();
        usdc = new ScenarioERC20Decimals("Mock USDC", "mUSDC", 6);
        weth = new ScenarioERC20Decimals("Mock WETH", "mWETH", 18);
        riskModule = new ScenarioPerpRiskModule(address(usdc), 6);
        ...
        riskModule.setAccountRisk(MAKER, HEALTHY_EQUITY, 0, 0);
        riskModule.setAccountRisk(LIQUIDATOR, HEALTHY_EQUITY, 0, 0);
    }

    function testTraderDepositOpenAdverseMoveLiquidateAndImproveSolvency() external {
        _depositUsdc(TRADER, 1_000 * BASE_UNIT);
        ...
        PerpEngineTypes.Position memory liquidatorPos = engine.positions(LIQUIDATOR, marketId);
    ...
    function testLiquidationConsumesCollateralSeizerPlanWhenAvailable() external {
        ...
        assertEq(vault.balances(TRADER, address(weth)), 0);
        assertEq(vault.balances(LIQUIDATOR, address(weth)), 1 ether);
        assertEq(engine.getResidualBadDebt(TRADER), 0);
    }
    ...
    function testLiquidationUsesInsuranceFundWhenSeizedCollateralIsInsufficient() external {
        ...
        uint256 liquidatorBefore = vault.balances(LIQUIDATOR, address(usdc));
        ...
        assertEq(
            vault.balances(LIQUIDATOR, address(usdc)),
    ...
    function testLiquidationRecordsResidualBadDebtWhenCollateralAndInsuranceAreInsufficient() external {
        _depositUsdc(TRADER, REALIZED_TRANSFER_BASE_ONE);
        _depositWeth(TRADER, 1 ether);
    ...
    function testHealthyAccountLiquidationAttemptReverts() external {
        _depositUsdc(TRADER, 1_000 * BASE_UNIT);
        _openLong(TRADER, MAKER, TWO);
