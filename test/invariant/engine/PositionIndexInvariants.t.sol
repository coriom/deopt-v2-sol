...
contract InvariantMarginRiskModule is IRiskModule {
    ...
    contract PositionIndexInvariantsTest is StdInvariant, Test {
        ...
        function setUp() external {
            ...
            marginEngine =
                new MarginEngine(OWNER, address(optionRegistry), address(vault), address(new InvariantEngineOracle()));
            ...
            optionIds.push(
                optionRegistry.createSeries(
                    address(weth), address(usdc), uint64(block.timestamp + 30 days), 2_000 * 1e8, true, true
                )
            );
            optionIds.push(
                optionRegistry.createSeries(
                    address(wbtc), address(usdc), uint64(block.timestamp + 45 days), 30_000 * 1e8, false, true
                )
            );
            ...
            marketIds.push(
                perpRegistry.createMarket(
                    ...
                    PerpMarketRegistry.LiquidationConfig({
                        closeFactorBps: 5_000, priceSpreadBps: 80, minImprovementBps: 50, oracleMaxDelay: 60
                    }),
                    PerpMarketRegistry.FundingConfig({
                        isEnabled: false,
                        fundingInterval: 0,
            ...
            handler = new PositionIndexInvariantHandler(
                vault, usdc, marginEngine, perpEngine, MATCHING_ENGINE, optionIds, marketIds
            );
        ...
        function invariant_nonZeroPerpPositionImpliesActiveMarketIndexPresence() external view {
            for (uint256 actorIndex = 0; actorIndex < ACTOR_COUNT; actorIndex++) {
                address actor = _actor(actorIndex);
                uint256[] memory active = _perpTraderMarkets(actor);
        ...
        function invariant_noDuplicateActiveSeriesOrMarketEntriesCanAppearForTrader() external view {
            for (uint256 actorIndex = 0; actorIndex < ACTOR_COUNT; actorIndex++) {
                ...
                uint256[] memory activeMarkets = _perpTraderMarkets(actor);
        ...
        function invariant_openInterestRemainsCoherentWithAggregateLivePerpPositionsUnderTestedSequences()
            external
            view
        {
            for (uint256 marketIndex = 0; marketIndex < MARKET_COUNT; marketIndex++) {
        ...
        function _perpTraderMarkets(address actor) internal view returns (uint256[] memory) {
            return perpEngine.getTraderMarketsSlice(actor, 0, perpEngine.getTraderMarketsLength(actor));
        }
