...
The product decisions this doc surfaced (`ACTIVE_SERIES_BOUND_REQUIRES_PRODUCT_DECISION`,
`LIQUIDATION_REMAINS_DISABLED_PENDING_COLLATERAL_COMPLETENESS`,
`COLLATERAL_LIQUIDATION_COMPLETENESS_REQUIRES_EXTENSION`) are now CLOSED
by `ONCHAIN-SUBACCOUNT-RISK-EXECUTION-BOUNDS-AND-COLLATERAL-UNIVERSE-V1`.
Frozen values: `MAX_ACTIVE_SERIES_PER_SUBACCOUNT = 32`,
`MAX_COLLATERAL_TOKENS = 8`. `READY_FOR_ONCHAIN_SUBACCOUNT_MARGIN_ENGINE_V2_V1`
is now issuable.
