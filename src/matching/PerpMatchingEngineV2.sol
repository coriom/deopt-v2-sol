...
/// @title PerpMatchingEngineV2
/// @notice V2 matching engine for perpetuals. Identical wire semantics to V1
///         (EIP-712 + nonces + executor gate) EXCEPT the domain separator is
///         bumped to version `"2"` so V1 signatures cannot verify on V2 and
///         V2 signatures cannot verify on V1.
/// @dev
///  Domain isolation (per PERPS_V2_SOLIDITY_FIX_AND_TESTS_V1 §7b):
///    V1: EIP712("DeOptV2-PerpMatchingEngine", "1")
///    V2: EIP712("DeOptV2-PerpMatchingEngine", "2")
///  combined with a fresh `verifyingContract` (V2 deploys at a new address)
///  guarantees replay separation. Enforced by
///  `test/matching/PerpMatchingEngineV2Domain.t.sol`.
///
///  The V2 matching engine wires to `PerpEngineV2` via the same
///  `IPerpEngineTrade` interface — no engine ABI change is required.
///  Executors, executor allowlist, nonce management, intent lifecycle,
///  and partial-fill accounting are unchanged from V1.
///
///  V1 file `PerpMatchingEngine.sol` is semantically untouched.
///
...
contract PerpMatchingEngineV2 is ReentrancyGuard, EIP712 {
    /*//////////////////////////////////////////////////////////////
                                EVENTS
    //////////////////////////////////////////////////////////////*/
    ...
    constructor(address _owner, address _engine) EIP712("DeOptV2-PerpMatchingEngine", "2") {
